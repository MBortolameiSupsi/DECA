...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            # Start measuring time for decoding
            ...
            #landmarks2D = convert_normalized_to_pixels(landmarks2D, image_width, image_height, testdata[i])
            landmarks2D = convert_normalized_to_pixels(landmarks2D, 224, 224, testdata[i])
            # Make the arrays contiguous
            ...
            #assert not np.any(np.isnan(landmarks3D)) and not np.any(np.isnan(landmarks2D))
            #assert not np.any(np.isinf(landmarks3D)) and not np.any(np.isinf(landmarks2D))
            ...
            if args.render_orig:
                print("render_orig ", args.render_orig)
...
##        -- save results
    ...
    for i in range(len(testdata)):
        ...
        print(f'Distance is {distances[i]}')

    print(f"Total encoding time for all images: {total_encode_time} seconds")
    print(f"Total decoding time for all images: {total_decode_time} seconds")
    print(f"Average encoding time per image: {total_encode_time / len(testdata)} seconds")
    print(f"Average decoding time per image: {total_decode_time / len(testdata)} seconds")   
    
    np.savez('./demos/all_landmarks3D.npz', all_landmarks3D = all_landmarks3D)
    np.savez('./demos/all_landmarks2D.npz', all_landmarks2D = all_landmarks2D)
